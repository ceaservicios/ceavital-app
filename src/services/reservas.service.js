...
// (vencido = fecha de vencimiento anterior a hoy en Argentina).
export async function stockVendible(productoId) {
  const fila = await db
    .prepare(
...
export async function stockReservado(productoId, { excluirPedidoId = null } = {}) {
  const fila = await db
    .prepare(
...
export async function stockDisponible(productoId, opciones = {}) {
  const [vendible, reservado] = await Promise.all([stockVendible(productoId), stockReservado(productoId, opciones)]);
  return Math.max(0, vendible - reservado);
}

// Catálogo que ve el cliente en su portal: solo lo que puede pedir. Nunca
// expone costo ni ningún dato interno del producto.
export async function catalogoConDisponibilidad() {
  ...
  const filasReservado = await db
    ...
    .all();
  const reservado = new Map(filasReservado.map((f) => [f.producto_id, f.total]));
